...
const Card = ({ data }) => {
  ...
  return (
    <Link to={`/moviedetail/${data.id}`}>
      {data.poster_path ? (
        <img
          className="select-none w-full h-full max-h-[18rem] sm:max-h-[24rem] md:max-h-[19rem] object-cover "
          src={`${IMG_URL_500W}${data.poster_path}`}
          alt="thumbnail"
        />
      ) : (
        <div className=" w-full h-full max-h-[18rem] sm:max-h-[24rem] md:max-h-[19rem] bg-black flex justify-center items-center">
